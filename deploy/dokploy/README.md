...
2. In Dokploy, project **OpenMAIC** > compose **openmaic** > *Redeploy* pulls the new tag.
   Rollback = set `OPENMAIC_IMAGE_TAG=sha-<previous>` in the env tab and redeploy.
...
Optional: `ACCESS_CODE` (site password), `OPENMAIC_AGENT_RUNTIME_ENABLED=true` +
`MODEL_ROUTES` to turn on the Pro agent workbench, `OPENMAIC_PORT` / `TAILSCALE_BIND_IP`.
